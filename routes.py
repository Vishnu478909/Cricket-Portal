...
def login():
    if request.method == 'POST':  # If the form is submitted
        ...
        if not username or not password:
            flash("Please enter both username and password.")  # Flash error if fields are empty
            return render_template('login.html')  
        ...
        cur = conn.cursor()
        cur.execute("SELECT password FROM User WHERE username = ?", (username,))  # Query for password
        user = cur.fetchone()  
        conn.close()
...
def register():
    if request.method == 'POST':
        ...
        try:
            conn = get_db_connections()  
            conn.execute("PRAGMA journal_mode=WAL")  # Set WAL mode for improved concurrency
            cur = conn.cursor()
            ...
            flash("Database is busy. Please try again in a few moments.")  # if database is locked
            return redirect(url_for('register'))
        finally:
            if conn:
                conn.close() 
...
def Statistics():
    conn = get_db_connections() 
    cur = conn.cursor()
    cur.execute("SELECT Team, image_url, Information FROM Statistics")  # Query for statistics to extract infomations,images and team name
